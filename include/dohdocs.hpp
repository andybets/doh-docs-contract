...
namespace eosio {
   ...
   class [[eosio::contract("eosio.dohdocs")]] dohdocs : public contract {
      public:
         ...
         ACTION adddoc(const uint64_t doc_id, const uint32_t faction_id, const uint32_t language_id, const name author, const std::string title, const std::string content);
         ...
         ACTION deldoc(const uint64_t doc_id, const uint32_t faction_id, const uint32_t language_id, const name author);
         ...
         ACTION publish(const uint64_t doc_id, const uint32_t faction_id, const uint32_t language_id, const name editor);

         // unpublishes a candidate tooltip document for the faction and language specified, and removes it from the published table
         ACTION unpublish(const uint64_t doc_id, const uint32_t faction_id, const uint32_t language_id, const name editor);
         ...
         static uint128_t composite_key_128(const uint64_t &doc_id, const uint32_t &faction_id, const uint32_t &language_id) {
             return (uint128_t{doc_id} << 64) | (uint128_t{faction_id} << 32) | language_id;
         }
      ...
      private:
         ...
         typedef eosio::multi_index< "published"_n, publisheddoc,
            indexed_by<"composite"_n, const_mem_fun<publisheddoc, uint128_t, &publisheddoc::by_composite_key> > > publisheddocs_table;
